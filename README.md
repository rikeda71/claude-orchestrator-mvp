...
- 通信パイプの作成
- 初期プロンプトの送信
- 統合ビューアの起動（デフォルト、`--no-viewer` で無効化可能）

### 1.5. 統合ビューアについて

**デフォルトで起動**: システム起動時に統合ビューアが自動的に新しいターミナルウィンドウで起動します。

すべてのセッションを1つの画面で監視できる統合ビューアの操作：
...
# ビューアなしで起動したい場合
./scripts/start-system.sh --no-viewer
...
```bash
# タスク作成
task create feature "ユーザー認証機能の実装"

# タスク一覧表示
task list

# タスクをエンジニアに割り当て
task assign task-001 eng1
```
